...
def test_price는_반올림이지_절단이_아니다():
    """결함7(2026-08-21 무동작 감사): `_price` 의 `round(v)` 를 `int(v)` 로
    바꿔도(뮤테이션) 기존 테스트는 전부 통과했다 — `0.4`(거부, round·int
    둘 다 결과가 같은 판정으로 이어짐)와 `100.0`(나누어떨어져 round·int가
    같은 값을 냄) 뿐이었기 때문이다. 이 라인의 주석 자체가 "절단(int)이
    아니라 반올림 — 계통적 편향 방지"라고 명시하는데, 정작 그 결정을
    지키는 테스트가 없었다 — orders.py 의 같은 이름 테스트(round↔int 가
    실제로 다른 값을 내는 입력을 쓴다)와 같은 이유로, round 와 int 가
    다른 결과를 내면서도 둘 다 여전히 ">0"을 통과하는 값을 쓴다."""
    assert models._price(2.6) == 3   # int(2.6) 이면 2 — 다른 결과로 드러난다
